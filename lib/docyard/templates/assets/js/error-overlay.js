(function () {
  ...
  function storeState(dismissed, lastErrorCount) {
    try {
  ...
  function shouldStartExpanded() {
    ...
    // Re-expand only if error count increased (new errors appeared)
    return errorCount > stored.lastErrorCount;
  }
  ...
  function renderToast() {
    ...
    if (errorCount > 0) {
      html.push('<span class="docyard-error-toast__item has-errors"><i class="ph ph-siren"></i>' + errorCount + ' error' + (errorCount !== 1 ? 's' : '') + '</span>');
    }
    ...
    if (warningCount > 0) {
      html.push('<span class="docyard-error-toast__item has-warnings"><i class="ph ph-warning"></i>' + warningCount + ' warning' + (warningCount !== 1 ? 's' : '') + '</span>');
    }
  ...
  function updateGlobalDiagnostics(globalDiags) {
    ...
    // Re-expand if new errors appeared
    if (!state.expanded && errorCount > prevErrorCount) {
      state.expanded = true;
      storeState(false, errorCount);
    }
